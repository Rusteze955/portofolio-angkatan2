...
if (isset($_GET['edit'])){
    ...
    if (isset($_POST['save'])){
        ...
        $update = mysqli_query($config, "UPDATE sumary SET name='$name', description='$description' WHERE id='$id'");
        header("location:?page=sumary&edit=berhasil");
    }   
}else{
    if (isset($_POST['save'])){
        $name = $_POST['name'];
        ...
        $insert = mysqli_query($config, "INSERT INTO sumary (name, description) VALUES ('$name', '$description')");
        header("location:?page=sumary&tambah=berhasil");
    }
}
